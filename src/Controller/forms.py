...
class PositionForm(FlaskForm):
	...
	start_date = StringField('Start Date', validators=[DataRequired(), Length(1, 8)])
	end_date = StringField('End Date', validators=[DataRequired(), Length(1, 8)])
	workload = IntegerField('Hours Per Week', validators=[DataRequired(), NumberRange(1, 168)])
	languages = QuerySelectMultipleField('Tag', query_factory=languages, get_label=get_language_label, widget=ListWidget(prefix_label=False), option_widget=CheckboxInput())
	requirements = QuerySelectMultipleField('Tag', query_factory=interests, get_label=get_interest_label, widget=ListWidget(prefix_label=False), option_widget=CheckboxInput())
